...
from PowerfulDiceRoller import DiceThrown, DiceError


class TesterDiceThrown(TestCase):
    ...
    def test_actual_throw(self):
        ...
        self.assertEqual(DiceThrown(throw=1, face=20).throw, 1)

    def test_actual_face(self):
        """значение Dice.face сответвует поданному числу"""
        self.assertEqual(DiceThrown(throw=1, face=20).face, 20)

    def test_roll_1d20(self):
        """если кидать 1Кчто-то-там, то должно вернуть число"""
        self.assertEqual(DiceThrown(throw=1, face=20).result, [5])
    ...
    def test_raise_if_use_part_retain(self):
        ...
        with self.assertRaises(DiceError):
            self.assertEqual(DiceThrown(throw=1, face=20, retain_f=lambda data: data[0]).result, 5)
